...
The main folder, where the README file is located, stores all the primary code that the user needs to run. These include: <br>
...
resolution, data-handling methods, and machine learning models. <br/><br/>

## 2. The_analysis.py
This script is essentially the Main_Machine script, designed to generate summaries that examine the effects 
of variables on performance, as discussed in the article. The variable names are explicit. <br/><br/>

## 3. Results_Analysis.inpynb
This Jupyter Notebook script creates almost all the figures present in the article. 
However, it's important to note that the code itself is stored in the Results_Analysis_Functions.py file. <br/><br/>
<br/><br/>
...
To create a new All_the_IR file, users can run the download.py script.<br/><br/>
...
algorithm works and to produce categorical/denoising algorithms. The code for creating these plots is located in the Jupyter Notebook Addtional Plot Making.ipynb.<br/><br/>
...
are created with all the necessary information for future use.<br/><br/>
...
the SMILES_SMARTS_string.json file, which must remain intact. Besides the scripts that run the machine learning model, the folder contains scripts for writing a summary. <br/><br/>
